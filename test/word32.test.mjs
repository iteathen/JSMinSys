...
test('caller-ply odd-center omission fully packs eligible one-lane state', () => {
  const columns = 5;
  const rows = 4;
  ...
  const packedDelta = (column) => {
    ...
    return 1 << (cellCount + 3 * compressedColumn);
  };
  ...
  for (let column = 0; column < columns; column += 1) {
    base[CALLER_PLY1_PLAYABLE_LO] |= 1 << column;
    packed[CENTER_OMIT1_META] |= 1 << column;
  }
  ...
  for (let move = 0; move < 3; move += 1) {
    ...
    assert.equal(
      playableFromCenterOmittedAll32(packed[CENTER_OMIT1_META], playableMask),
      base[CALLER_PLY1_PLAYABLE_LO],
    );
    assert.equal(
      supportFromCenterOmittedAll32(packed[CENTER_OMIT1_META], cellCount),
      compressSupport(base[CALLER_PLY1_SUPPORT_CODE]),
    );
  }
  ...
  const compressedSupport = supportFromCenterOmittedAll32(
    packed[CENTER_OMIT1_META], cellCount,
  );
  const reflectedFull = reflectPacked3Direct32(
    base[CALLER_PLY1_SUPPORT_CODE], columns, 12,
  );
  assert.equal(
    reflectPacked3Columns4(compressedSupport),
  ...
  const knownBase = new Uint32Array(2);
  const knownPacked = new Uint32Array(1);
  const knownLandingBase = new Uint32Array(columns);
  const knownLandingPacked = new Uint32Array(columns);
  fillLandingCells32(knownLandingBase, columns);
  fillLandingCells32(knownLandingPacked, columns);
  knownBase[CALLER_PLY1_PLAYABLE_LO] = (1 << columns) - 1;
  knownPacked[CENTER_OMIT1_META] = (1 << columns) - 1;

  applyMove1x32CallerPlyKnownCell(
    knownBase, knownLandingBase, sideColumn, sideColumn,
    columns, cellCount, 1 << (3 * sideColumn),
  );
  applyMove1x32CallerPlyCenterOmittedKnownCell(
    knownPacked, knownLandingPacked, sideColumn, sideColumn,
    columns, cellCount, packedDelta(sideColumn),
  );
  assert.equal(
    playableFromCenterOmittedAll32(knownPacked[CENTER_OMIT1_META], playableMask),
    knownBase[CALLER_PLY1_PLAYABLE_LO],
  );
  assert.equal(
    undoMove1x32CallerPlyCenterOmittedKnownCell(
      knownPacked, knownLandingPacked, sideColumn, sideColumn,
      columns, cellCount - columns, packedDelta(sideColumn),
    ),
    undoMove1x32CallerPlyKnownCell(
      knownBase, knownLandingBase, sideColumn, sideColumn,
      columns, cellCount - columns, 1 << (3 * sideColumn),
    ),
  );
...
test('fully packed one-lane state matches separate transition state', () => {
  const columns = 3;
  const rows = 4;
  const cellCount = columns * rows;
  const rankBits = 4;
  const rankMask = (1 << rankBits) - 1;
  const playableMask = (1 << cellCount) - 1;
  const supportShift = rankBits + cellCount;
  const supportDelta = 1 << (1 * 3);
  const combinedDelta = 1 + (supportDelta << supportShift);
  ...
  const landingOrdinary = new Uint32Array(columns);
  const landingPacked = new Uint32Array(columns);
  fillLandingCells32(landingOrdinary, columns);
  fillLandingCells32(landingPacked, columns);
  ordinary[STATE_PLAYABLE_LO] = 0b111;
  packed[PACKED_ALL1_META] = 0b111 << rankBits;

  assert.equal(
    applyMove1x32(
      ordinary, landingOrdinary, 1, columns, cellCount, supportDelta,
    ),
    1,
  );
  assert.equal(
    applyMove1x32PackedAll(
      packed, landingPacked, 1, columns, cellCount, rankBits, combinedDelta,
    ),
    1,
  );
  assert.equal(
    playableFromPackedAll32(packed[PACKED_ALL1_META], rankBits, playableMask),
    ordinary[STATE_PLAYABLE_LO],
  );
  assert.equal(
    supportFromPackedAll32(packed[PACKED_ALL1_META], supportShift),
    ordinary[STATE_SUPPORT_CODE],
  );
  assert.equal(
    plyFromPackedRankLow32(packed[PACKED_ALL1_META], rankMask),
    ordinary[STATE_PLY],
  );
  assert.equal(sideFromPackedRankLow32(packed[PACKED_ALL1_META]), 1);
  ...
  assert.equal(packed[PACKED_ALL1_META], 0b111 << rankBits);
  assert.deepEqual(landingPacked, landingOrdinary);
  ...
  const knownLandingOrdinary = new Uint32Array(columns);
  const knownLandingPacked = new Uint32Array(columns);
  fillLandingCells32(knownLandingOrdinary, columns);
  fillLandingCells32(knownLandingPacked, columns);
  knownOrdinary[STATE_PLAYABLE_LO] = 0b111;
  knownPacked[PACKED_ALL1_META] = 0b111 << rankBits;

  applyMove1x32KnownCell(
    knownOrdinary, knownLandingOrdinary, 1, 1, columns, cellCount, supportDelta,
  );
  applyMove1x32PackedAllKnownCell(
    knownPacked, knownLandingPacked, 1, 1, columns, cellCount,
    rankBits, combinedDelta,
  );
  assert.equal(
    playableFromPackedAll32(knownPacked[PACKED_ALL1_META], rankBits, playableMask),
    knownOrdinary[STATE_PLAYABLE_LO],
  );
  assert.equal(
    undoMove1x32PackedAllKnownCell(
      knownPacked, knownLandingPacked, 1, 1, columns, cellCount - columns,
      rankBits, combinedDelta,
    ),
    undoMove1x32KnownCell(
      knownOrdinary, knownLandingOrdinary, 1, 1, columns, cellCount - columns,
      supportDelta,
    ),
  );
...
test('5-column reflection fused final alignment preserves edge fields', () => {
  const vectors = [
    0,
    7,
    7 << 12,
    (7 | (6 << 3) | (5 << 6) | (4 << 9) | (3 << 12)) >>> 0,
    0x7fff,
  ];
  for (const code of vectors) {
    assert.equal(
      reflectPacked3Columns5(code),
      reflectPacked3Direct32(code, 5, 12),
    );
  }
});
