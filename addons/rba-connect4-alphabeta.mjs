...
function probeConnect4RbaExactCacheSlot32(cache,words,offset,slot,hash){
  ...
  if(cache.shared){
    if(!(hash&cache.sharedSampleBits)||
       (words[offset+cache.sharedMetaOffset]>>>2)>=36)
      return probeConnect4RbaSharedExactCache32(cache.shared,words,offset);
  }
  return 0;
}
...
export function prepareConnect4RbaAlphaBeta({
  ...
  cache.sharedMetaOffset=g.metaOffset;
