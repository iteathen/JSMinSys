...
export function prepareConnect4RbaFrontArena(g,{depth=2,capacity=256,budget=100000}={}){
  ...
  return {depth,capacity,budget,steps:0,error:0,actionBase,recordWords,
...
function insert(a,slot){
  ...
  const n=insertMinimalSpan32InPlace(a.words,a.base[slot],a.count[slot],a.capacity,a.recordWords,a.temp,0);
...
function combine(a,left,right,out,intersect){
  ...
  const n=productJoinMinimalSpan32Into(a.words,a.base[out],0,a.capacity,a.words,a.base[left],a.count[left],a.words,a.base[right],a.count[right],a.recordWords,a.temp,0);
...
function prepareUpsets(g,a,d,basis,bi,n){
  for(let i=0;i<n;i+=1){const row=(d*g.maxBasis+i)*g.coordWords;for(let w=0;w<g.coordWords;w+=1)a.up[row+w]=0;
    for(let j=0;j<n;j+=1)if(connect4RbaShapeSubset(g,basis[bi+i],basis[bi+j]))a.up[row+(j>>>5)]|=1<<(j&31);
  }
}
function prepareImages(g,a,d,cell,mover,basis,bi){
  const n=a.size[d],cn=a.size[d+1];
  for(let i=0;i<n;i+=1){const id=basis[bi+i],removed=connect4RbaRemoveCell(g,id,cell);a.top1[i]=0;
    ...
    for(let p=0;p<2;p+=1){if(p!==mover&&removed!==id)continue;const image=p===mover?removed:id,out=p===0?a.image0:a.image1;
      ...
      else for(let j=0;j<cn;j+=1)if(connect4RbaShapeSubset(g,image,a.basis[(d+1)*g.maxBasis+j]))out[row+(j>>>5)]|=1<<(j&31);
    }
  }
}
...
function buildAt(g,a,d,remaining,basis,bi,n){
  ...
  for(let c=0;c<g.columns;c+=1){const height=a.heights[c];if(height>=g.rows)continue;const cell=height*g.columns+c,childBi=(d+1)*g.maxBasis;
    const cn=connect4RbaCofactorBasis(g,basis,bi,n,cell,a.basis,childBi,a.seen);a.heights[c]=height+1;
...
function member(g,a,slot,words,offset){
  const cw=g.coordWords;for(let i=0;i<a.count[slot];i+=1){const b=a.base[slot]+i*a.recordWords;let ok=1;
    for(let w=0;w<cw;w+=1)if((a.words[b+w]&~words[offset+g.p0Offset+w])||(a.words[b+cw+w]&words[offset+g.p1Offset+w])){ok=0;break;}
    if(ok)return 1;
  }return 0;
}
