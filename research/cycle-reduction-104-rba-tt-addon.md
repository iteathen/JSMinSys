...
## Runtime-configured carrier

The original round-104 checkpoint selected an eight-word / 69-basis / seven-edge
carrier from the 7x6 proving position. That selection was corrected before merge.
The add-on now receives key width, basis capacity, and dependency capacity from
application initialization. Connect4 RBA geometry derives those values from the
configured board dimensions.

Fixed-width helpers remain optional measured specializations; they are not the
RBA/TT contract.

## Owned by the add-on

- exact runtime-width interning with full equality after locator selection;
...
- the semantic meaning of the configured q words;
...
`createRbaTt32` is cold. Application adoption must still run the transitive
